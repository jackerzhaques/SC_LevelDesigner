    ...
    ui(new Ui::SpriteCreator)
{
    ...
    this->FullSheetView         = this->ui->FullSpritesheetView;
    this->SpriteEditView        = this->ui->SpriteEditView;
    this->SpritePreview         = this->ui->AnimationPreview;
...
void SpriteCreator::FullSpritesheetClicked(QPoint pos)
{
    QPointF MappedPos = this->FullSheetView->mapToScene(pos);
    this->HandleClick(MappedPos);
}

void SpriteCreator::ZoomedSpritesheetClicked(QPoint pos)
{
    QPointF MappedPos = this->SpriteEditView->mapToScene(pos);
    this->HandleClick(MappedPos);
}

void SpriteCreator::FullSpritesheetMouseMoved(QPointF pos)
{
    QPointF mappedPos = this->FullSheetView->mapToScene(pos.toPoint());
    this->HandleDrag(mappedPos);
}

void SpriteCreator::ZoomedSpritesheetMouseMoved(QPointF pos)
{
...
void SpriteCreator::on_AddFrameButton_released()
{
    this->AddSprite();
}

void SpriteCreator::AddSprite()
{
    auto *NewSprite = new Sprite(this);
    if(this->CurrentSprite){
        this->SaveCurrentSprite();
        this->CurrentSprite->CopyTo(NewSprite);
    }

    auto type = static_cast<Sprite::SpriteType>(this->ui->SpriteTypeCombo->currentIndex());
    auto dir = static_cast<Sprite::Direction>(this->ui->SpriteDirCombo->currentIndex());
...
void SpriteCreator::AddSprite(Sprite::SpriteType type, Sprite::Direction dir,
...
void SpriteCreator::ClearSprites()
{
...
void SpriteCreator::RemoveSelectedSprite()
{
    int typeIndex = this->ui->SpriteTypeCombo->currentIndex();
    int dirIndex = this->ui->SpriteDirCombo->currentIndex();
    int spriteIndex = this->ui->SpriteList->currentRow();
...
void SpriteCreator::RefreshList()
{
    this->ui->SpriteList->clear();
    int typeIndex = this->ui->SpriteTypeCombo->currentIndex();
    int dirIndex = this->ui->SpriteDirCombo->currentIndex();
    QString typeLabel = this->TypeStrings->at(typeIndex);
    QString dirLabel = this->DirStrings->at(typeIndex);
...
void SpriteCreator::SaveCurrentSprite()
{
...
void SpriteCreator::AdjustSelectionRect(QPointF target)
{
...
void SpriteCreator::AdjustZoomedView()
{
...
void SpriteCreator::keyPressEvent(QKeyEvent *keyEvent)
{
...
void SpriteCreator::SavePlacingObjectPos()
{
...
void SpriteCreator::SetPlacingObjectPos()
{
    switch(this->PlacingMode){
        case ANCHOR:
...
void SpriteCreator::SetPlacingMode(SpriteCreator::PlaceMode mode)
{
...
void SpriteCreator::HandleClick(QPointF pos)
{
    switch(this->PlacingMode){
        case NORMAL:
...
void SpriteCreator::HandleDrag(QPointF pos)
{
    switch(PlacingMode){
        case ANCHOR:
...
void SpriteCreator::SetEllipseObjectPos(QGraphicsEllipseItem *item, QPointF pos)
{
...
void SpriteCreator::SetRectObjectPos(QGraphicsRectItem *item, QPointF pos)
{
...
void SpriteCreator::SetAnimation()
{
    this->CurrentFrame = 0;
}
 ...
 */
void SpriteCreator::ScaleAnimatedSprite(double ViewPercentage)
{
...
QString SpriteCreator::ConvertToXML()
{
    QString xml;
...
void SpriteCreator::OpenImage(const QString &filePath)
{
...
void SpriteCreator::OpenXML(const QString &filePath)
{
    QDomDocument doc;
    QFile file(filePath);
    if(!file.open(QIODevice::ReadOnly)){
        ...
        while(!child.isNull()){
            ...
            //Cast to their enums
            auto type = static_cast<Sprite::SpriteType>(typeIndex);
            auto dir = static_cast<Sprite::Direction>(dirIndex);
...
QPointF SpriteCreator::StringToPointF(QString string)
{
    ...
    auto nullPoint = QPointF(0,0);
    return nullPoint;
}

QRectF SpriteCreator::StringToRectF(QString string)
{
    //Remove parenthesis in front and back
    string.remove(0,1);
    string.remove(string.size() - 1, 1);
...
void SpriteCreator::on_SpriteList_currentRowChanged(int currentRow)
{
    if(currentRow >= 0){
        this->SaveCurrentSprite();
...
void SpriteCreator::on_SpriteDirCombo_currentIndexChanged(int index)
{
    this->SetAnimation();
    this->RefreshList();
}

void SpriteCreator::on_OpenSpriteButton_released()
{
    QString fileName = QFileDialog::getOpenFileName(this, "Open File",
                                 "P:/QtProjects/Gingerspice/LevelDesigner/Resources/Spritesheets",
...
void SpriteCreator::on_PlaceModeCombo_currentIndexChanged(int index)
{
    auto mode = static_cast<PlaceMode>(index);
    this->PlacingMode = mode;
}

void SpriteCreator::on_SpriteTypeCombo_currentIndexChanged(int index)
{
    this->SetAnimation();
    this->RefreshList();
}

void SpriteCreator::on_AutoplayCheck_toggled(bool checked)
{
    this->AutoplayEnabled = checked;
}

void SpriteCreator::on_HideOriginCheck_toggled(bool checked)
{
    if(checked){
...
void SpriteCreator::on_ScaleSpriteSlider_valueChanged(int value)
{
    double perc = static_cast<double>(value) / 100.0;
    this->AnimationViewPercentage = perc;
}

void SpriteCreator::on_RemoveFrameButton_released()
{
    this->RemoveSelectedSprite();
}

void SpriteCreator::on_SaveAsButton_released()
{
    //Generate XML
...
void SpriteCreator::on_SpritePeriodInput_valueChanged(int arg1)
{
